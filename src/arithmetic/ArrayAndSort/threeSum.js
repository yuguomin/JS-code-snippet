 ...
 * 2. 排序后，从0开始遍历，对它之后的内容，建立头尾两个指针，left 为 i + 1 和 right 为 len - 1;也就是当前遍历的下一个数和最后一个数，
 ...
 * 6. 这样相当于以所有不大于0的值为基础遍历，查看后续组合是否有戳和项，并根据排序后特点对重复项进行过滤，
 */
  ...
  for (let i = 0; i < numsLen; i++) {
    ...
    while(left < right) {
      ...
      if (sum === 0) {
        ...
        while(right > left && numR === nums[right - 1]) right--;
        left++;
        right--;
      } else if (sum < 0) {
        left++;
